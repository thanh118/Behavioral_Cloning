...
class Pipeline:
    ...
    def process_batch(self, batch_sample):
        sterring_angle = np.float32(batch_sample[3])
        images, sterring_angles = [], []

        for images_path_index in range(3):
            source_path = batch_sample[images_path_index]
            ...
            if images_path_index == 1:
                sterring_angles.append(sterring_angle + self.correction_factor)
            elif images_path_index == 2:
                sterring_angles.append(sterring_angle - self.correction_factor)
            else:
                sterring_angles.append(sterring_angle)

            if images_path_index == 0:
                flipped_center_image = utils.flipimg(resized)
                images.append(flipped_center_image)
                sterring_angles.append(-sterring_angle)
        return images, sterring_angles
...
def main():
    ...
    parser.add_argument(
        '--data-base-path',
        type = str,
        default= '/home/thanh/Desktop/data',
